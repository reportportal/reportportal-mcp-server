...
| Create Test Case | Creates a new test case with a TEXT manual scenario type. **Mutates TMS data.** | `name` (required), `description` (optional), `priority` (optional, enum: `LOW` \| `MEDIUM` \| `HIGH` \| `CRITICAL` \| `BLOCKER` \| `UNSPECIFIED`), `test-folder-id` (optional, integer ≥ 1), `instructions` (optional), `expected-result` (optional), `preconditions` (optional), `requirements` (optional, array of value strings; a unique id is generated automatically per entry) |
