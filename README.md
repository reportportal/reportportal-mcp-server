...
| Create Test Case | Creates a new test case. Use `test-case-type` to choose the manual scenario. **Mutates TMS data.** | `name` (required), `description` (optional), `priority` (optional, enum: `LOW` \| `MEDIUM` \| `HIGH` \| `CRITICAL` \| `BLOCKER` \| `UNSPECIFIED`), `test-folder-id` (optional, integer ≥ 1), `test-case-type` (optional, enum: `description` (default) \| `test case with steps`), `instructions` (optional, for `description` type), `expected-result` (optional, for `description` type), `steps` (optional, array of `{ instructions, expected-result }`, for `test case with steps` type), `preconditions` (optional), `requirements` (optional, array of value strings; a unique id is generated automatically per entry) |
