...
**Note:** On **Get Test Items by filter**, optional `filter-eq-defect-type` limits results to a defect/issue type (ReportPortal `filter.eq.issueType`). You can pass built-in labels or refs (for example Product Bug or `PRODUCT_BUG`) or a project-specific locator from **Get Project Defect Types**.
