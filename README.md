...
| Create Test Case | Creates a new test case with a TEXT manual scenario type. **Mutates TMS data.** | `projectKey` (required), `name` (required), `description` (optional), `priority` (optional, enum: `LOW` \| `MEDIUM` \| `HIGH` \| `CRITICAL`), `test-folder-id` (optional, integer ≥ 1), `instructions` (optional), `expectedResult` (optional) |
| Create Milestone | Creates a new milestone in the TMS. **Mutates TMS data.** Dates must be RFC3339 (e.g. `2026-01-01T00:00:00Z`); `endDate` must not precede `startDate`. | `projectKey` (required), `name` (required), `type` (required, enum: `SPRINT` \| `RELEASE` \| `OTHER`), `startDate` (required, RFC3339), `endDate` (required, RFC3339), `status` (optional, enum: `ACTIVE` \| `CLOSED`) |
