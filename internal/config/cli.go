...
import (
	...
	analyticslib "github.com/reportportal/reportportal-mcp-server/internal/analytics"
...
func GetCommonFlags() []cli.Flag {
	return []cli.Flag{
		...
		&cli.StringFlag{
			...
			Usage:    "[GLOBAL/REQUIRED] API token for authentication",
		},
		&cli.StringFlag{
			Name:     "project",
			Required: false,
		...
		&cli.StringFlag{
			...
			Value:    "",
			Usage:    "[GLOBAL/OPTIONAL] Unified user ID for analytics (empty = auto-generate)",
		},
		&cli.BoolFlag{
			Name:     "analytics-off",
			Required: false,
...
func BuildHTTPServerConfig(
	...
	analyticsAPISecret := analyticslib.GetAnalyticArg()
...
func NewMCPServer(
	cmd *cli.Command,
	appVersion AppVersion,
) (*server.MCPServer, *analyticslib.Analytics, error) {
	...
	analyticsAPISecret := analyticslib.GetAnalyticArg() // Analytics API secret
	...
	mcpServer, analytics, err := mcp_handlers.NewServer(
		appVersion.Version,
		hostUrl,
		token,
		project,
...
func HandleServerError(
	err error,
	analyticsClient *analyticslib.Analytics,
	serverType string,
) error {
	...
	if err == nil ||
		...
		analyticslib.StopAnalytics(analyticsClient, "")
		return nil
	}

	slog.Error("server error occurred", "type", serverType, "error", err)
	analyticslib.StopAnalytics(analyticsClient, "server error")
...
func RunStdioServer(ctx context.Context, cmd *cli.Command, appVersion AppVersion) error {
	mcpServer, analytics, err := NewMCPServer(cmd, appVersion)
	if err != nil {
	...
	case <-ctx.Done(): // Context canceled (e.g., SIGTERM received)
		...
		analyticslib.StopAnalytics(analytics, "")
...
func RunStreamingServer(ctx context.Context, cmd *cli.Command, appVersion AppVersion) error {
	...
	httpServer, analytics, err := httpserver.CreateHTTPServerWithMiddleware(httpConfig)
	if err != nil {
...
USAGE EXAMPLES:
   ...
   reportportal-mcp-server --rp-host https://rp.example.com --token abc123

   # Run in http mode with custom port
   MCP_MODE=http reportportal-mcp-server --rp-host https://rp.example.com --token abc123 --port 9090`,
		Flags:  allFlags,
		Before: InitLogger(),
